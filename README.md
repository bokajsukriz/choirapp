...
Unter dem Normalisierungs-Schalter listet „Übersprungene und fehlgeschlagene Dateien" jede betroffene Spur mit Dateiname, Song/Stimme (falls bekannt), Status und einem konkreten, ehrlichen Grund (Dauer-/Größen-/PCM-Grenze, fehlende/ungültige Metadaten, Timeout, fehlende Decoder-Unterstützung, Decoder-Fehler, Speicherfehler oder ein offen benannter „unbekannter Grund") — nie eine geratene Pauschalaussage wie „nicht unterstützt" oder „beschädigt". Technische Fehlernamen (z. B. eine `DOMException`) stehen nur in einem ausklappbaren Detailbereich, nie in der Hauptzeile. Gespeichert werden ausschließlich kleine strukturierte Diagnosedaten (Grundcode, Messwerte, Policy-Version, Quellenrevision); der Text wird bei jedem Rendern frisch in der aktuellen UI-Sprache erzeugt, sodass die Liste nach einem Neustart korrekt rekonstruiert wird und bei Sprachwechsel sofort mitzieht. Fehlgeschlagene Dateien haben einen eigenen „Erneut versuchen"-Knopf.
